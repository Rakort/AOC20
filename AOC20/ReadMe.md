...
Парсим вхожные данные в массив кортежей (операция, аргумент). 
...
Выполняем инструкции пошагово сохраняя индекы посещенных строк и если встречаем повторение возвращаем аккумулятор.
...
Что поможет получать изменненые значения без изменения исходного массива или создания копии массива с одним изменненым значением.
...
Если индекс меньше длины массива увеличиваем индекс опрерации для замены и пытаемся снова.
...
Когда находи не действиленьный элемент это и будет наш ответ.
...
И повторяем снова убрав перевый элемент из списка пока не наедем нужную последовательность.

Найдя требуемую непрерывную последовательность для вычисления уязвимость шифрования находим минимальный и максимальный элемент и складываем.
```fsharp
